...
The shuffled version has different nodes id. However, we expect to find the same communities in g adn gs.
```{r}
plot(gs, vertex.size = 25)
```
on a single trial, it may seem that shuffling the network does not change the results.
...
# 
comms_g <- cluster_leiden(g, resolution = .99)
comms_gs <- cluster_leiden(gs, resolution = .99)
plot(comms_g,g, main = "Leiden - original network")
plot(comms_gs,gs, main = "Leiden - shuffled network")
```

```{r}
comms_g <- cluster_louvain(g, resolution = .99)
comms_gs <- cluster_louvain(gs, resolution = .99)
...
for (i in 1:1000) {
  gs <- igraph::permute(g, 
                              ...
                              size = vcount(g), 
                              replace = FALSE ) )
  center_id <- which(degree(gs)==4)
  
  #print(paste("The central node id is ", center_id))
  
  #comms <- cluster_louvain(gs, resolution = 0.99)
