...
summary_n_trials_cons %>% write.csv(paste0('./tmp/summary_',method,'.cvs'))
```
...
ok adesso fare le 3 strategie di outliers: 
incorporate, Higlight, group
...
# plots for paper
```{r}
s1<- read_csv(paste0('./tmp/summary_LP_inc.csv'))
s2<- read_csv(paste0('./tmp/summary_LP_high.csv'))
s3<- read_csv(paste0('./tmp/summary_LP_g.csv'))

summary_n_trials_cons <- rbind(s1,s2,s3)
summary_n_trials_cons <- summary_n_trials_cons %>%
  mutate(across("method", str_replace, "LP_ing_cons", "incorporate"))%>%
  mutate(across("method", str_replace, "LP_high_cons", "highlight"))%>%
  mutate(across("method", str_replace, "LP_g_cons", "group"))
...
grid.arrange(p1 , 
              p2 , 
              ncol = 2, widths=c(1,1.4) , heights= c(1,1), top = "label propagation")  
```
